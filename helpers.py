...
def setup(update, context):
    update.message.reply_text(
        'checking user settings...', reply_markup=keyboard)
...
def ask_notion_api_key(update, context):
    update.message.reply_text(
        'please send me an Notion API key', reply_markup=keyboard)
    return TYPING_NOTION_API_KEY


def set_notion_api_key(update, context):
    update.message.reply_text(
        'setting Notion API key...', reply_markup=keyboard)
...
def setclient(update, context, user):
    ...
    try:
        ...
        update.message.reply_text(
            '✓ Notion client set!', reply_markup=keyboard)
...
def check_client(update, context):
    ...
    if user.notion_api_key:
        update.message.reply_text(
            '✓ Notion API key set!', reply_markup=keyboard)

    if not user.notion_api_key:
        update.message.reply_text(
            '☢ Notion API key not set.', reply_markup=keyboard)
    ...
    if not context.user_data.get('notion_client'):
        update.message.reply_text(
            '☢ Notion client not set.', reply_markup=keyboard)
        setclient(update, context, user)
...
def connect_to_page(update, context, user, page_address):
    try:
        ...
        update.message.reply_text(
            f'✓ connected to database {user.page_title}!')
...
def check_page(update, context):
    ...
    if user.page_address:
        update.message.reply_text(
            f'✓ page address set.', reply_markup=keyboard)

    if not user.page_address:
        update.message.reply_text(
            '☢ page address not set.', reply_markup=keyboard)
    ...
    if not context.user_data.get('page'):
        update.message.reply_text(
            '☢ page not connected.', reply_markup=keyboard)
        connect_to_page(update, context, user, user.page_address)
